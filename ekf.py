from quaternion import Quaternion, ndarrayToQuat
import numpy as np 
...
if __name__ == '__main__':
    ...
    ekf_euler: np.ndarray = np.zeros(shape=(n, 3))

    ekf = FullStateEKF(gyro_noise=0.001, accel_noise=0.1, freq=data.freq, initial_state=initial_q)

    for i in range(n):
        q, _ = ekf.update(data.gyro[i, :], data.accel[i, :])
        ekf_euler[i, :] = q.toEuler()

    for i in range(3):
        plt.figure()
        plt.plot(euler[:, i])
        plt.plot(ekf_euler[:, i])
        plt.legend(['acc', 'full state ekf'])
